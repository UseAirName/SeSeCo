...
def setup_model(device):
    ...
    if config.network == 'sum':
        ...
        model.load_state_dict(torch.load('./encoder_decoder/sum_saliency/net/pre_trained_weights/sum_model.pth', map_location=device))
        model.to(device)
        return model
    else:
...
def inference(img_path):
    ...
    pred_saliency = predict_saliency_map(img, 0, model, device)
